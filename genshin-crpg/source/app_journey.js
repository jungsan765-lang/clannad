...
dialogue=function(p,v){journeyDialogue(p,v);if(!game.atGuild())return;const w=game.s.worldProgress,t=w.training,used=t.day===game.s.global.WORLD_DAY?t.count:0,c=el('section','card guild-training');c.append(el('h2','','길드 훈련'),el('p','','편성 중인 정식 파티원 전원에게 경험치 250 · 50 모라 · 10초'),el('p','muted','오늘 '+used+' / 3회 · 게임 시간 30분'),actionButton('기초 전투 훈련','WORLD_WORK_START',{kind:'TRAIN'},true));
 c.append(el('h3','','주인공 무술 숙련 '+w.mastery+' / 2'),el('p','','기본공격 피해 단계당 +30% · 필요한 레벨 '+(3+w.mastery*2)));if(w.mastery<2)c.append(el('p','',100*(w.mastery+1)+' 모라 · 철광석 '+3*(w.mastery+1)+'개'),actionButton('다음 무술 숙련 배우기','MASTERY',{},true));p.append(c);
};
