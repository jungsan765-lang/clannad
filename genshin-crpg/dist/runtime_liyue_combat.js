...
P.azhdahaCenter=function(a,fallback){
 const allies=this.s.runtime?.actors.filter(t=>t.side!==a.side&&t.hp>0)||[],z=allies.find(t=>t.source==='LIYUE_ZHONGLI');
 if(a.azhdaha?.zhongliBound&&z)return z;const chosen=allies.find(t=>t.id===fallback);if(chosen)return chosen;
 return allies.slice().sort((x,y)=>this.combatStat(y,'atk')-this.combatStat(x,'atk')||y.hp-x.hp)[0]||null;
};
P.cardTargets=function(a,c){
 if(isAzhdaha(a)&&['ECARD_AZHDAHA_QUAKE','ECARD_AZHDAHA_ELEMENT_ERUPTION','ECARD_AZHDAHA_TAILSTORM'].includes(c.id)){const all=this.s.runtime.actors.filter(t=>t.side!==a.side&&t.hp>0),center=this.azhdahaCenter(a);return center?[center,...all.filter(t=>t!==center)]:all;}
 if(['ECARD_FATUI_ANEMO_GUARD','ECARD_FATUI_GEO_BARRIER','ECARD_FATUI_HYDRO_HEAL'].includes(c.id))return this.s.runtime.actors.filter(t=>t.side===a.side&&t.hp>0&&(c.id==='ECARD_FATUI_HYDRO_HEAL'?t.hp/t.maxHp<=.7:!t.shields.some(s=>s.value>0))).sort((x,y)=>x.hp/x.maxHp-y.hp/y.maxHp);return old.cardTargets.call(this,a,c);};
P.cardReason=function(a,c){if(c.id==='ECARD_FATUI_AGENT_STEALTH'&&a.statuses.some(s=>s.id==='AGENT_STEALTH'))return '이미 잠행 중입니다.';if(['ECARD_FATUI_ANEMO_GUARD','ECARD_FATUI_GEO_BARRIER','ECARD_FATUI_HYDRO_HEAL'].includes(c.id)&&!this.cardTargets(a,c).length)return '지원할 대상이 없습니다.';if(['ECARD_MITA_ROCK_SHIELD','ECARD_RUIN_VARIANT_CORE','ECARD_FATUI_CRYO_ARMOR','ECARD_FATUI_ELECTRO_ARMOR'].includes(c.id))return '조건에 따라 자동 발동합니다.';if(c.id==='ECARD_MITA_ROCK_CHARGE'&&!a.shields.some(s=>s.value>0))return '바위 방패가 필요합니다.';if(this.s.runtime?.liyueInterlude)return '전투 중 대화를 먼저 확인하세요.';if(c.enemy&&bossIds.has(c.owner)){
 if(isOsial(a)||c.trigger.startsWith('AUTO_')||['ECARD_AZHDAHA_ELEMENT_CORE','ECARD_AZHDAHA_ZHONGLI','ECARD_AZHDAHA_PHASE_SHIFT'].includes(c.id))return '전투 기믹으로 자동 처리됩니다.';
 if(isAzhdaha(a)){if(c.id==='ECARD_AZHDAHA_ELEMENT_ERUPTION'&&(!a.azhdaha||a.azhdaha.current==='GEO'))return '원소 주입 이후 사용합니다.';if(c.id==='ECARD_AZHDAHA_TAILSTORM'&&a.hp/a.maxHp>.6)return 'HP 60% 이하에서 사용합니다.';}
 const phase=Number(c.trigger.match(/ACTIVE_IF_PHASE=(\d)/)?.[1]);if(phase&&phase!==(a.liyuePhase||1))return '현재 단계에서 사용하지 않습니다.';if(a.cooldowns[c.id]>0)return '재사용 대기 중입니다.';return '';}return old.cardReason.call(this,a,c);};
P.executeCard=function(a,c,target,branch){
 ...
 if(isAzhdaha(a)){
  const allies=this.s.runtime.actors.filter(t=>t.side==='ALLY'&&t.hp>0),center=this.azhdahaCenter(a,target),current=a.azhdaha?.current||'GEO';
  if(c.id==='ECARD_AZHDAHA_QUAKE'){for(const v of allies)this.damage(a,v,.95,'GEO',{range:'전장',card:c.id,aoe:true});if(center?.hp>0)this.damage(a,center,.35,'GEO',{range:'전장',card:c.id,noAura:true,aoe:true});}
  else if(c.id==='ECARD_AZHDAHA_ELEMENT_ERUPTION'){for(const v of allies){if(v.hp>0)this.damage(a,v,.70,current,{range:'전장',card:c.id,aoe:true});if(v.hp>0)this.damage(a,v,v===center?.875:.70,current,{range:'전장',card:c.id,noAura:true,aoe:true});}}
  else if(c.id==='ECARD_AZHDAHA_TAILSTORM'){if(current==='GEO'){for(const v of allies)this.damage(a,v,1.45,'GEO',{range:'전장',card:c.id,aoe:true});}else for(const v of allies){if(v.hp>0)this.damage(a,v,.725,'GEO',{range:'전장',card:c.id,aoe:true});if(v.hp>0)this.damage(a,v,.725,current,{range:'전장',card:c.id,noAura:true,aoe:true});}if(center?.hp>0)this.damage(a,center,.55,current,{range:'전장',card:c.id,noAura:true,aoe:true});}
  else return;
  a.cooldowns[c.id]=c.cooldown;this.s.runtime.log.push({actor:a.name,actorId:a.id,card:c.id,cardName:c.name,round:this.s.runtime.round,actionSequence:this.s.runtime.actionSequence||0});return;
 }
...
P.newRound=function(){
 ...
 for(const a of b.actors.filter(isAzhdaha))if(!a.azhdaha){
  const pool=['PYRO','HYDRO','CRYO','ELECTRO'],first=pool.splice(this.die(pool.length)-1,1)[0],second=pool[this.die(pool.length)-1],zhongli=b.actors.some(t=>t.side==='ALLY'&&t.source==='LIYUE_ZHONGLI'&&t.hp>0);
  a.azhdaha={version:1,element1:first,element2:second,current:'GEO',phase:1,zhongliBound:zhongli,neutralUsedRound:0};
  b.log.push({actor:a.name,actorId:a.id,card:'ECARD_AZHDAHA_ELEMENT_CORE',cardName:'용왕의 원소 주입',text:'야타용왕이 흡수할 원소를 정했다: '+elementKo(first)+' → '+elementKo(second),round:b.round});
  if(zhongli)b.log.push({actor:a.name,actorId:a.id,card:'ECARD_AZHDAHA_ZHONGLI',cardName:'옛 계약의 구속',text:'종려의 존재로 야타용왕의 공격력·방어력이 감소하고 광역 중심점이 종려에게 고정된다.',round:b.round});
 }
...
P.damage=function(a,t,k,e,o={}){
 if(isAzhdaha(t)&&t.azhdaha?.current!=='GEO'&&elementCode(e)===t.azhdaha.current&&!o.neutralConverted){
  const b=this.s.runtime,canNeutral=!o.sourceKind&&a.control==='PLAYER'&&this.s.preparedTools?.includes('TRPG_TACTICAL_ELEMENT_CONVERTER')&&t.azhdaha.neutralUsedRound!==b.round;
  if(canNeutral){t.azhdaha.neutralUsedRound=b.round;b.log.push({actor:a.name,actorId:a.id,target:t.name,targetId:t.id,text:'원소 공명 변환기 · 중성 변환. 이번 직접 공격을 물리/중성으로 바꾸고 최종 피해가 70%가 된다.',round:b.round,actionSequence:b.actionSequence||0});return old.damage.call(this,a,t,k*.70,'PHYSICAL',{...o,neutralConverted:true,noAura:true});}
  b.log.push({actor:a.name,actorId:a.id,target:t.name,targetId:t.id,damage:0,immune:'현재 주입 원소 완전면역',element:e,round:b.round,actionSequence:b.actionSequence||0});return false;
 }
 if(isOsial(t)){this.s.runtime.log.push({actor:a.name,target:t.name,damage:0,immune:'본체 무적 · 진법을 지키세요.'});return false;}
...
P.addCombatStatus=function(a,id,...rest){if(isOsial(a))return null;if(isAzhdaha(a)&&a.azhdaha?.current!=='GEO'&&id==='STATUS_FREEZE'&&['CRYO','HYDRO'].includes(a.azhdaha.current))return null;return old.addCombatStatus.call(this,a,id,...rest);};
