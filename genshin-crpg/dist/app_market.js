...
shop=function(p,v){
 ...
 const categories=['전체',...new Set(all.map(e=>e.group))];if(!categories.includes(marketCategory))marketCategory='전체';for(const category of categories){const b=button(category,()=>{marketCategory=category;marketSelection=null;render();});b.classList.toggle('selected',category===marketCategory);filter.append(b);}p.append(filter);
