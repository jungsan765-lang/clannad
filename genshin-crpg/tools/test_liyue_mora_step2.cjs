...
for(const [map,group] of cases)test('Liyue random victory grants shared Mora: '+map+' / '+group,()=>{const r=fresh(map);const x=settle(r,group);assert(x.plan&&x.plan.mora>0);assert.equal(x.delta,x.plan.mora);assert.equal(x.result.mora,x.plan.mora);assert.equal(x.result.rewardPolicy,'LIYUE_FIELD_MORA_V1');assert.equal(x.result.moraMap,map);assert.equal(x.result.moraRisk,x.plan.risk);assert.deepEqual(clone(x.result.moraParts),clone(x.plan.parts));assert.equal(r.s.combatReceipts[x.id].mora,x.plan.mora);report.samples.push({map,group,mora:x.plan.mora,risk:x.plan.risk,parts:clone(x.plan.parts)});return{mora:x.plan.mora,risk:x.plan.risk};});
...
test('defeat grants no Mora even with the regional reward plan recorded',()=>{const r=fresh();const x=settle(r,'EG_TREASURE_PATROL','RANDOM',false);assert.equal(x.delta,0);assert.equal(x.result.mora,0);assert.equal(x.result.rewardPolicy,'LIYUE_FIELD_MORA_V1');return{delta:x.delta};});
