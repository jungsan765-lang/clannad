...
test('defeat gives no direct Mora',()=>{const x=settle('MAP_LIYUE_PLAINS','EG_TREASURE_PATROL',{victory:false,die:1});assert.equal(x.result.mora,0);assert.equal(x.delta,-Math.floor(x.before*.1),'only the v0.13.30 defeat penalty');return{mora:x.delta};});
