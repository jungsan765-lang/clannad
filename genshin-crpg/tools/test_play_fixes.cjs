...
function stack(withFixes=true){const c=vm.createContext({console,Date:class extends Date{static now(){return clock;}},setTimeout,clearTimeout,TextEncoder,TextDecoder});const html=fs.readFileSync(dir+'/index.html','utf8');for(const[,f]of html.matchAll(/<script src="((?:world_content|liyue_card_content|runtime[^" ?]*)\.js)(?:\?[^" ]*)?"/g))if(withFixes||f!=='runtime_play_fixes.js')vm.runInContext(fs.readFileSync(dir+'/'+f,'utf8'),c,{filename:f});vm.runInContext(fs.readFileSync(dir+'/journal_presenter.js','utf8'),c,{filename:'journal_presenter.js'});return c;}
const ctx=stack(),R=ctx.CRPGRuntime.Runtime,J=ctx.CRPGJournalPresenter,DB=JSON.parse(raw),R0=stack(false).CRPGRuntime.Runtime,DB0=JSON.parse(raw);
const report={version:'0.13.32',runtimeDir,scope:'personal-story exits and travel, fallen protagonist wins, defeat penalty, enemy target spread, equipment and forge screens, item wording and icons',checks:[]};
...
test('equipment screen, party notice and defeat timer are wired (source)',()=>{const read=f=>fs.readFileSync(path.join(dir,f),'utf8'),html=read('index.html'),gear=read('app_gear.js'),party=read('app_party.js'),app=read('app.js'),css=read('style.css');
 ...
 assert(!html.includes('\\n<script'),'no stray \\n in index.html');const at=f=>html.indexOf('"'+f);assert(at('runtime_play_fixes.js')>at('runtime_liyue_encounters.js')&&at('runtime_play_fixes.js')<at('save_adapter.js'));assert(at('app_gear.js')>at('app_liyue_areas.js'));const scripts=[...html.matchAll(/<script src="([^"?]+)/g)].map(m=>m[1]);assert(scripts.indexOf('app_gear.js')>=scripts.length-3,'app_gear.js loads among the last modules');
...
// v0.13.32: stories and travel goals, Barbara's dish name, forge list.
function mainTravel(r){const row=r.storyIndex().byTable['55_MAIN_STORY_DB'].find(x=>x[0]==='ROUTE_TRAVELER'&&x[18]==='ACTIVE'&&x[8]==='MAP_MOND_PLAINS'&&['DIALOGUE','NARRATION'].includes(x[5])&&!/EVENT:/.test(x[12]));Object.assign(r.s.global,{CURRENT_STORY_NODE_ID:row[4],STORY_CURSOR_NODE_ID:row[4],SCREEN_MODE:'STORY'});r.prepareStory();return row[4];}
function introduce(r,id){const d=r.storyDefinition(id);r.s.flags.FLAG_TRV_MON_PROLOGUE_CLEAR=true;r.s.guildLegends={...(r.s.guildLegends||{}),[id]:{day:1}};r.s.global.MORA=99999;for(const [k,v]of Object.entries(JSON.parse(d.COST_ITEMS_JSON||'{}')))r.giveItem(k,v);return d;}
function advance(r,onTravel){const moves=[];for(let n=0;n<150&&r.s.storyContext;n++){if(r.s.storyJourney){const j=r.s.storyJourney;moves.push(j.target);onTravel?.(j);r.s.global.CURRENT_MAP_ID=j.target;r.action('JOURNEY_RESUME');continue;}if(r.s.global.PENDING_CHOICE_GROUP_ID){const ch=r.storyChoices(),pick=ch.find(x=>/LEGEND_ACCEPT_AND_PAY/.test(x[12]))||ch[0];r.action('STORY_CHOICE',{node:pick[4],choice:true});continue;}const node=r.storyNode();assert(node&&node[5]!=='COMBAT_GATE','story stopped at '+r.storyActiveNodeId());r.action('STORY_NEXT',{node:node[4]});}return moves;}
test('companion missions start during main-story travel and the travel goal comes back afterwards',()=>{const r=fresh(),node=mainTravel(r),main=JSON.stringify(r.s.storyJourney);assert(r.s.storyJourney&&!r.s.storyJourney.scripted);assert.equal(r.playPhase(),'FREE');
 const id='LEG_MOND_AMBER';introduce(r,id);assert.equal(r.actionReason('LEGEND_ENTER',{quest:id}),'');r.action('LEGEND_ENTER',{quest:id});
 assert.equal(r.playPhase(),'STORY');assert(!r.s.storyJourney,'main goal waits in the return frame');assert.equal(JSON.stringify(r.s.storyReturnStack.at(-1).journey),main);assert.equal(new R(DB,JSON.parse(r.serialize())).playPhase(),'STORY');
 const moves=advance(r);assert(r.storyDone(id));assert.equal(JSON.stringify(r.s.storyJourney),main);assert.equal(r.playPhase(),'FREE');
 r.s.global.CURRENT_MAP_ID=r.s.storyJourney.target;assert.equal(r.actionReason('JOURNEY_RESUME'),'');r.action('JOURNEY_RESUME');assert.equal(r.storyActiveNodeId(),node);return {mainGoal:JSON.parse(main).target,missionMoves:moves};});
test('a companion mission paused for travel reads as in progress and says where it continues',()=>{const r=fresh(),id='LEG_MOND_AMBER';introduce(r,id);r.action('LEGEND_ENTER',{quest:id});const seen=[];
 advance(r,j=>{const entry=r.storyEntries().find(e=>e.id===id),state=L(J.progress(r,entry)),legend=L(J.inProgress(r).legends.find(m=>m.id===id)),reason=r.actionReason('AFFECTION_ENTER',{event:'AFF_X'});
  assert.deepEqual(state,{status:'active',reason:'',map:j.target,arrived:false});assert.equal(legend.status,'active');assert.match(reason,/엠버 전설 임무/);assert(reason.includes(r.row('32_MAP_DB',j.target)[2]),reason);assert.match(reason,/도착 · 이야기 계속/);
  const there=Object.create(r);there.s={...r.s,global:{...r.s.global,CURRENT_MAP_ID:j.target}};assert.equal(J.progress(there,entry).arrived,true);seen.push(j.target);});
 assert.deepEqual(seen,['MAP_MOND_PLAINS','MAP_MOND_CITY']);assert.equal(J.progress(r,r.storyEntries().find(e=>e.id===id)).status,'done');return {moves:seen};});
test('guided moves still keep other stories closed until they finish',()=>{const r=fresh(),node=mainTravel(r);introduce(r,'LEG_MOND_AMBER');assert.equal(r.actionReason('LEGEND_ENTER',{quest:'LEG_MOND_AMBER'}),'');for(const special of [{scripted:true},{special:'DVALIN_RIDE'}]){r.s.storyJourney={...r.s.storyJourney,...special};assert.equal(r.actionReason('LEGEND_ENTER',{quest:'LEG_MOND_AMBER'}),'안내에 따른 이동을 마친 뒤 다른 이야기를 시작해 주세요.');}return {node};});
test('Barbara\'s special dish keeps the dish name first and explains itself',()=>{const r=fresh(),egg='FOOD_TEA_BREAK_PANCAKE';r.giveItem(egg,2);r.s.specialFoodLots={[egg]:{BARBARA_SPECIAL:1}};
 assert.deepEqual(L(r.foodLots(egg).map(l=>l.name)),['티바트 달걀 프라이','티바트 달걀 프라이 · 바바라 특제']);assert.equal(r.foodSpec(egg,{variant:'BARBARA_SPECIAL'}).name,'티바트 달걀 프라이 · 바바라 특제');
 const P=vm.createContext({});vm.runInContext(fs.readFileSync(path.join(dir,'inventory_presenter.js'),'utf8'),P);const d=P.CRPGInventoryPresenter.create(DB,{}).itemDetail({item:egg,quantity:1},{variant:'BARBARA_SPECIAL'});
 assert.equal(d.name,'티바트 달걀 프라이 · 바바라 특제');assert.match(d.fields.find(f=>f.label==='바바라 특제')?.value||'',/정성♪듬뿍.*10%/);assert(!P.CRPGInventoryPresenter.create(DB,{}).itemDetail({item:egg,quantity:1}).fields.some(f=>f.label==='바바라 특제'));return {name:d.name};});
test('forge shortage messages use the right Korean particle',()=>{const r=fresh();r.s.global.MORA=999999;const items=Object.keys(ctx.CRPGRuntime.enhancementConfig.ascensionCost.items).filter(id=>!r.itemCount(id)),msgs=items.map(id=>r.enhancementMissing({mora:0,items:{[id]:1}}));assert(msgs.length);for(const m of msgs)assert.match(m,/^\S.*[^)](이|가) 부족합니다\.$/,m);return msgs;});
test('forge lists gear compactly with category tabs and filters (source)',()=>{const read=f=>fs.readFileSync(path.join(dir,f),'utf8'),forge=read('app_enhancement.js'),art=read('app_liyue_artifacts.js'),css=read('style.css');for(const f of ['app_enhancement.js','app_liyue_artifacts.js','app_adventure.js','app_experience.js','journal_presenter.js'])new vm.Script(read(f),{filename:f});
 assert(forge.includes('forgeList(section)')&&forge.includes("['ARTIFACT','성유물']")&&forge.includes("'지금 강화할 수 있는 것만'")&&!forge.includes("'enhance-grid'"));assert(art.includes('globalThis.CRPGArtifactForge')&&!art.includes('renderEnhancementPanel'));assert(css.includes('.forge-row')&&css.includes('.journal-card.status-active'));
 const adv=read('app_adventure.js');assert(adv.includes('function journalContinue')&&adv.includes("'도착 · 이야기 계속','JOURNEY_RESUME'"));});
