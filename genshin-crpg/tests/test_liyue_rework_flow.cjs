...
function walk(r,target,run){const route=r.navigationRoute(target);if(!route?.edges.length)throw Error('no legal path '+r.s.global.CURRENT_MAP_ID+' -> '+target);const row=route.edges[0];run.last={move:row[0],from:row[1],to:row[2],field:r.s.liyueField?.mission};r.action('MOVE',{edge:row[0]});}
...
for(const leaf of targets){let r;const run={leaf,actions:0,restores:[],battles:[],chapters:[],choices:[],status:'RUNNING'};result.runs.push(run);flush();try{
 ...
 for(let i=0;i<7000;i++){
  ...
  if(r.s.liyueField){
   const view=r.liyueFieldView(),f=view.state,step=view.step;run.last={field:f.mission,stage:f.stage,target:view.target,map:r.s.global.CURRENT_MAP_ID};
